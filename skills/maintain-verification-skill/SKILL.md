...
0. **Locate the target.** Search canonical user skills under `~/.agents/skills/verify-*/`, then an explicitly configured project-local `.agents/skills/verify-*/` location. The target must contain launch and drive sections plus a feature map. Several candidates → ask which one; none → stop and point at `/create-verification-skill` instead of inventing a target. Never edit a compatibility link as an independent copy.
