...
1. Read the installed dstack capability contract under `DSTACK_HOME/contracts/` (`DSTACK_HOME` defaults to `~/.dstack`).
...
Do not invent tool parameters, model identifiers, isolation, or persistence. If a capability is absent or denied, use the documented parent fallback.
