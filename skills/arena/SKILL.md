...
3. Pick the runners. Use your configured `deep-judgment` role, with as many distinct model bindings as your profile provides. Spawn more when the arena covers multiple design directions. Same binding N times when the work is generation-bound rather than judgment-sensitive.
...
After all Phase B candidates complete, use your configured `independent-judge` role, falling back to `deep-judgment`. Prefer a binding distinct from the parent's. Spawn one read-only judge subagent on that binding. It sees the rubric and the candidates by path label, scores each criterion, and recommends a base with rationale. It runs in parallel with the parent's reading in Phase D, not with the candidates themselves. Spawning while candidates are still writing means the judge sees partial or empty outputs and reports them as dropouts.
