...
The orchestrator passes this file through to every parallel candidate runner during Phase B and fills in the variable inputs around it: the task, the Phase A grounding artifacts, the read-only repository path, and the isolated path for proposal outputs. Each runner writes only to its own proposal directory; the parent applies the selected design serially in the active checkout.
...
You are one of several runners, each assigned a configured profile. Produce the best design you can make; don't hedge against the others. Differences between candidates are the signal used to pick a base and graft. Converging on a safe-looking middle defeats the exploration.
