...
- tool access: full. **Don't use a restricted read-only mode.** Where a host strips MCP access in that mode it disables MCP-backed investigators entirely. The source control investigator would be safe restricted, but keep modes uniform. Investigators still shouldn't write anything. That's a posture, not a sandbox.
