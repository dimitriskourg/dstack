...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, editor selection, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list the MCP servers available in this session. Use the host's available-tools map when it exposes one. Otherwise inspect whatever enabled-MCP listing the host provides.
...
- tool access: full. **Don't use a restricted read-only mode.** Where a host strips MCP access in that mode it disables MCP-backed investigators entirely. Investigators still shouldn't write anything.
...
1. **Source control investigator**. Local Git history, available GitHub or GitLab request context, code comments, and tests. Always spawn. The only guaranteed source. Best at surfacing *implementation-time rationale captured during review*.
