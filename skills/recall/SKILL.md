...
Use the selected repository entry's configured `transcripts_directory`. Do not search for the directory again. When it is `null` or the directory is unreadable, say so up front and build the brief from steps 4 and 5 alone.

For every fan-out below, use the host's reported available child capacity when it exposes one; otherwise conservatively launch one child at a time. Drain each bounded wave before starting the next. A routed workflow such as `why` may spawn its own children: count that nested work against the same capacity, and run routed fan-out groups one at a time when the host does not expose enough nested capacity. Retry a denied or dropped required slice in a later wave, then complete it in the current agent if it still cannot run. Do not silently omit a slice. Report the number of parallel waves, any serialized fallback, and any lost independence.
...
3. Fan out across your chat history. Spawn subagents on a fast, cheap model across bounded waves, each taking a slice of the corpus, since searching transcripts is grunt work. Tell every subagent to order candidates by real modification time (`ls -t`) and never by UUID name, grep the topic first and then read only the matching chats and only their relevant regions, and skip the current chat plus obvious noise (subagent, eval, and test chats). Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the chat UUID. For one or two chats, skip the fan-out and search directly. The raw transcripts stay in the subagents. The main thread gets only their findings.
