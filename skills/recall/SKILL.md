...
Read `DSTACK_HOME/config.json`, defaulting `DSTACK_HOME` to `~/.dstack`, and use the active host's `transcripts_directory`. Do not search for the directory again. When it is `null`, missing, or unreadable, say so up front and build the brief from steps 4 and 5 alone.
