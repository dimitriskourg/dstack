...
5. Verify against live state. Take the pull requests, merge requests, branches, and tickets that the mining and the sweep surfaced and check them with `git` and, after resolving the configured remote, authenticated `gh` for GitHub or authenticated `glab` for GitLab. If the matching CLI is unavailable, verify local Git state and report forge status as a gap. When the answer hinges on what an agent actually did (the tools it ran, files it read, errors it hit), read the full transcript, not just a trimmed local copy.
