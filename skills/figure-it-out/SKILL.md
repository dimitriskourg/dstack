...
- For one-way-door design decisions, Call the Skill tool with `architect`. It calls `arena`. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
- Decide what fans out. Parallelize read-only exploration and independent artifacts outside the repository. Serialize repository writers in the active checkout (the **separate-before-serializing-shared-state** principle skill). Don't over-fan.
...
Call the Skill tool with `show-me-your-work`. One canonical TSV with a row per decision and per unit, evidence as links. figure-it-out's work is usually ambitious enough to commit the trail so the reviewer can read it in the PR. Commit it when confidence has to be shown. Prefer evidence produced by committed scripts. The trail plus the diff is what lets the human come back and trust the work.
