...
Produce a phased implementation plan grounded in the **Principles** section of the `dstack-mode` skill. The plan is the deliverable. Do not implement.
...
Read the **Principles** section of the `dstack-mode` skill end to end, and the leaf `principle-*` skills it indexes. The principles govern every plan decision; cross-link them.

## 2. Scope and constraints

State your read of scope and constraints in one paragraph. Stop to ask only for genuinely ambiguous intent (the **never-block-on-the-human** principle skill); give concrete options with each open question.
...
If a phase creates or edits a skill, the phase instructs the implementer to use your host's skill-authoring skill.
...
- Browser, Electron, and web UIs: the control skill your team publishes for browser surfaces.
- CLIs and TUIs: the control skill your team publishes for terminal surfaces.
...
In the overview, name which dstack-mode non-negotiables the implementer must apply, by name:
