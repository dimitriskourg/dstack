...
# operated in it (when AGENT_TRANSCRIPTS_DIR is set). Emits a table sorted by
# size with a suggested bucket. Never
...
# Transcript dir for the LAST_CHAT column. Hosts store these differently, so
# point AGENT_TRANSCRIPTS_DIR at your host's directory for this workspace.
# Unset, the column reports "-" and every other column still works.
transcripts="${AGENT_TRANSCRIPTS_DIR:-}"
now=$(date +%s)
