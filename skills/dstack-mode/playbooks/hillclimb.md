...
1. Ground the workload and architecture before choosing the ruler. Call the Skill tool with `how`. Apply it to the target, name the realistic workload dimensions that can move the result, and select a case that reproduces the user's complaint. If no case reproduces it, fix the repro instead of hillclimbing. Then fix one metric, the direction that counts as better, and a checkable stop predicate.
...
3. Call the Skill tool with `show-me-your-work`. Keep one `decision.tsv` row per attempt with id, hypothesis, change, before, after, delta, tests, verdict, and note. Read it before each attempt. Keep it out of the tree so it survives reverts.
