...
1. Stop at a safe boundary. Finish the current atomic step or back out of it. Never stop mid-edit in a known-broken state. Start nothing new, and cancel any nested helpers.
...
4. Write the resume note outside conversation context. Capture intent, progress, verification, current state, next steps, key files, and gotchas. Put it under a host-provided safe temporary directory or an explicit repository scratch location, because an in-context plan may not survive compaction. If a show-me-your-work trail exists, point at it instead of duplicating it.
