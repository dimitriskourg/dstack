...
**You own a clean stop. Leave a checkpoint a cold-start agent can resume from.** For "pause safely", "I need to go offline", "restart your editor", or "board my flight", and when context is about to compact or summarize. This is explicit only. On "keep going" or "don't stop", do not pause; continue only while the active session can supervise the task.
