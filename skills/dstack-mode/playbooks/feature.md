...
4. Delegate code-writing to a subagent using the configured `feature-worker` profile with a specific scope and success criteria; review its diff yourself. When the implementation admits multiple valid shapes, Call the Skill tool with `arena`. Let the runners surface alternatives and the cross-judge guard the pick. Every supported harness can spawn subagents. If a nested spawn is denied, the current agent owns the diff directly. Comments per **Comments**. Make surgical edits, re-ground against upstream-derived sources, port shared-primitive improvements to all consumers, and verify each.
