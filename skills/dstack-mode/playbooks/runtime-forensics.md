...
1. Capture the live signal on the matching surface. For a CLI or TUI, Call the Skill tool with `control-cli`. For a browser, IDE, or Electron surface, Call the Skill tool with `control-ui`. Capture a real artifact, not a guess.
