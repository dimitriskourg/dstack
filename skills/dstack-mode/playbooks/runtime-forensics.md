...
1. Capture the live signal on the matching surface. For a CLI or TUI, Call the Skill tool with `control-cli`. For a browser, IDE, or Electron surface, Call the Skill tool with `control-ui`. Capture a CPU profile for a spinning process, a heap snapshot for a leak, a CDP trace for a visual glitch. A real artifact, not a guess.
