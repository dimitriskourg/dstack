...
**You own the design decision, not the code. The prototype is a throwaway instrument. The real build follows Feature.** Also for settling an empirical fork (which behavior, which timing, which approach) by observing it run, when you would otherwise ask the human a question a quick sketch could answer for you.
...
5. Verify on the matching surface. For a visual decision, Call the Skill tool with `control-ui`. Screenshot each variant while driving the interaction. For CLI or TUI behavior, Call the Skill tool with `control-cli`. For other timing decisions, observe the thing you are deciding by logging the timing, printing the output, or watching the render. The observation is the test here, not an assertion.
