...
4. Verify each component against its baseline via image diff on the real UI surface available through `verify`. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Use the host's long-running or loop mechanism per component until the diff is zero.
