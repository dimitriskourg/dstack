...
1. Reproduce it yourself on the matching surface. For a CLI or TUI, Call the Skill tool with `control-cli`. For a browser, IDE, or Electron surface, Call the Skill tool with `control-ui`. Don't hand the repro to the user. A debug or instrumentation protocol that says to ask the user does not override this. You drive the instrumented runtime. Ask the user only with a stated, specific reason the available control surface cannot reach the target, and only after driving it as far as it goes. Won't reproduce directly, force it: synthesize the trigger, tighten conditions, or instrument until it fires. A bug you cannot reproduce cannot be proven fixed.
